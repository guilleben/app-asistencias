...
export type MaterialListPdfMeta = {
  id: number;
  date: string;
  owner: string;
  workName: string;
...
function buildFilename(meta: MaterialListPdfMeta): string {
  return buildMaterialListPdfFilename({
...
export type DownloadMaterialListPdfResult =
  | { ok: true; method: "share" | "download" | "open" }
  | { ok: false; message: string };

export async function downloadOrShareMaterialListPdf(
  meta: MaterialListPdfMeta,
): Promise<DownloadMaterialListPdfResult> {
  let response: Response;

  try {
    response = await fetch(`/api/listado-materiales/${meta.id}/pdf`);
  } catch {
  ...
  if (!response.ok) {
    if (response.status === 404) {
      return { ok: false, message: "Listado no encontrado" };
    }
